...
def bfs(x,y,graph,visitedGraph,isSick): #isSick true -> 색맹/ O(10000)
    color = graph[x][y]
    visitedGraph[x][y] = 1 #방문
...
for x in range(1,n+1):
    for y in range(1,n+1):
        if(visitedGraph[x][y] == 0): #방문 아직 안했다면
            notSickNum+=1
            bfs(x,y,graph,visitedGraph,False)
...
for x in range(1,n+1):
    for y in range(1,n+1):
        if(visitedGraph[x][y] == 0): #방문 아직 안했다면
            sickNum+=1
            bfs(x,y,graph,visitedGraph,True)
